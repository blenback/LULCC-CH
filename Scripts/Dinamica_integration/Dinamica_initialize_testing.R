...
Scenario_transition_matrix_folder <- str_replace((list.files("Data/Transition_tables/prepared_trans_tables", pattern = Scenario_ID, full.names = TRUE)), paste0(wpath, "/"), "")
...
### =========================================================================
### F- LULC map initialization + glacier conversion   
### =========================================================================
...
if(Scenario_start <= 2020){
  ...
  if(grepl("simulation", Model_mode, ignore.case = TRUE)){
      ...
      #1st step: replace the current LULC values with the updated values 
      #(11: Static for non-glacier cells and 19 for glacier cells) matching the
      #IDs from the Glacier index
