...
current_LULC_path <- paste0(File_path_simulated_LULC_maps, Simulation_time_step)
...
system.time({ #Currently 9 mins to complete prediction generation
...
rm(pred_data, prob_predicts, Final_LULC, Initial_LULC, Trans_ID, Trans_name)
...
#Match on columns containing "Prob_"
Prediction_probs[,grep("Prob_", names(Prediction_probs), value = TRUE)] <- as.data.frame(t(apply(Prediction_probs[,grep("Prob_", names(Prediction_probs), value = TRUE)], MARGIN = 1, FUN = function(x) {
  sapply(x, function(y) {
    value <- y*1/sum(x)
...
#so far the process of prediction generation, re-scaling and raster saving takes 11 minutes
