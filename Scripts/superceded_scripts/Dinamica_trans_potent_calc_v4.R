...
if (grepl("simulation", Model_mode, ignore.case = TRUE)){
  Trans_data_stack <- stack(LULC_data, SA_pred_stack, stack(Nhood_rasters))
names(Trans_data_stack) <- c(names(LULC_data), names(SA_pred_stack@layers), names(Nhood_rasters))
} #close simulation if statement
