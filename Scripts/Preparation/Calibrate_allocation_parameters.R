...
# remove the X.1 and X columns
transitions <- transitions[, !names(transitions) %in% c("X.1", "X")]

# subset to From. == 11 to capture the static -> Shrubland transition as this
#is the only new transition that needs to be calibrated.
transitions <- transitions[transitions$From. %in% c(11,10), ]

# keep only rows where 'To.' is 14 or 15
transitions <- transitions[transitions$To. %in% c(14,15), ]
...
no_cores <- 1 # just one core as we are only doing the single new transition
...
#Apply function
# NOTE ONLY USING THE THIRD TIME PERIOD AS THIS IS THE ONE USED FOR SIMULATIONS
Allocation_params_by_period <- mapply(lulcc.periodicparametercalculation, Raster_combo = LULC_change_periods[3],
                                        period_name = names(LULC_change_periods)[3],
...
# Load the optimal allocation parameters for the simulation that were derived in the previous study
Optimal_params <- read.csv("Data/Allocation_parameters/Optimal_param_table.csv")

# Now add the params for the new transitions using the values estimated for the
# most recent historical period
Estimated_params <- read.csv("Data/Allocation_parameters/Calibration/Periodic/Allocation_parameters_2009_2018.csv")

# convert the Perc_expander and Perc_patcher columns to decimal format
Estimated_params$Perc_expander <- Estimated_params$Perc_expander / 100
Estimated_params$Perc_patcher <- Estimated_params$Perc_patcher / 100

# add the row of Estimated_params where From. == 11 and To. == 14 to the optimal params
param_table <- rbind(Optimal_params, Estimated_params)

# convert all columns to Double-precision floating-point format
param_table[] <- lapply(param_table, function(x) as.numeric(as.character(x)))

# change thename of the 'From.' and 'To.' columns to 'From*' and 'To*'
colnames(param_table)[1:2] <- c("From*", "To*")
...
sapply(Scenario_IDs, function(y){
  sapply(seq(Simulation_start, Simulation_end, Step_length), function(x){
    ...
    write.csv(param_table, file = file_name, row.names = FALSE)
  })  
})
